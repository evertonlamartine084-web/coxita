...
import { getPedidosPorTokens } from '../../services/orders'
import { lerTokens } from '../../utils/pedidosLocais'
...
import { linkDoPedido } from '../../utils/pedidosLocais'
...
export default function OrderHistoryPage() {
  ...
  // os códigos guardados neste aparelho; sem eles não há como listar pedido nenhum
  const [myOrderTokens] = useState(() => Object.values(lerTokens()))
  const navigate = useNavigate()
  const { addItem, clearCart } = useCartStore()

  useEffect(() => {
    if (myOrderTokens.length === 0) {
      setLoading(false)
      return
    }
    getPedidosPorTokens(myOrderTokens)
      .then(setOrders)
      .catch(console.error)
  ...
  return (
    <>
      <Seo titulo="Meus pedidos" caminho="/meus-pedidos" noindex />
              ...
              <div key={order.id} className="bg-white border-2 border-brown/20 shadow-[4px_4px_0_rgba(93,43,4,0.12)] overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="font-display font-bold text-lg text-primary">#{order.codigo_cliente ?? order.order_number}</span>
                  ...
                  <div className="px-4 pb-4 border-t border-gray-100">
                    ...
                    <div className="flex gap-2 mt-4">
                      {!['entregue', 'cancelado'].includes(order.status) && (
                        <Link
                          to={linkDoPedido(order.order_number)}
